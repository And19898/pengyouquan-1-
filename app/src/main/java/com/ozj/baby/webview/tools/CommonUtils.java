...
public class CommonUtils {
    ...
    public static Intent getInstallIntent(File apkFile) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
            String auth = BabyApplicationLike.get().getResources().getString(R.string.AutoUpdateprovide);
            uri = FileProvider.getUriForFile(BabyApplicationLike.get(), auth, apkFile);
...
//        return AndroidProcesses.isMyProcessInTheForeground();
    ...
    public static Class<Activity> getLaunchActivity() {
        try {
            PackageInfo packageInfo = BabyApplicationLike.get().getPackageManager().getPackageInfo(BabyApplicationLike.get().getPackageName(), 0);
            ...
            List<ResolveInfo> list = BabyApplicationLike.get().getPackageManager().queryIntentActivities(resovleIntent, 0);
